...
export default function ProjectCard({ project, ...aosProps }) {
  ...
  return (
    <>
      <div
        className="p-4 bg-zinc-800 rounded-md transition-all duration-300 
      ...
      >
        ...
        <div className="flex flex-col flex-1">
          <h1 className="text-base md:text-xl font-bold my-4">
            {project.nama}
          </h1>
          <p className="text-sm md:text-base/loose my-4 opacity-70">
            {project.desk}
          </p>
          <div className="flex flex-wrap gap-2">
            {project.tools.map((tool, index) => (
              <p
                className="py-1 px-3 text-sm rounded-md font-semibold bg-zinc-700"
                key={index}
              >
                {tool}
              </p>
          ...
          <div className="mt-auto pt-8 text-center">
            <div
              className={`flex ${project.linkweb ? "gap-3" : "justify-center"}`}
            >
              <a
                href={project.link}
                target="_blank"
        ...
        ${project.linkweb ? "flex-1" : "w-full max-w-[200px]"}`}
              ...
              {project.linkweb && (
                <a
                  ...
                  text-sm md:text-base bg-violet-700 p-3 rounded-lg border border-zinc-600 
                  hover:bg-violet-600 transition-colors flex-1"
                >
                  See Projects
